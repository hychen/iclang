...
1. At the topmost level is IClang's lexcial structure. The lexical structure represents ICLang programs in 
   JSON files.
2. At the top level is modules described in Chapter [[Modules][Modules]].
...
IClang modules are maintianed by conventional programming language package menagement tool. 
...
The *metadata.json* file must contain a node-red entry that identifies what nodes the module provides,
